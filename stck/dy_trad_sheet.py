...
def get_or_create_worksheet(workbook, sheet_name):
    """워크시트 객체를 반환하고, 없으면 생성합니다."""
    if workbook is None:
        return None
    try:
        return workbook.worksheet(sheet_name)
    except gspread.exceptions.WorksheetNotFound:
        st.info(f"'{sheet_name}' 워크시트가 없어 새로 생성합니다.")
        # rows와 cols는 적절히 조절 가능
...
# ============================================
# ✅ Google Sheet 종가 업데이트 로직 (price_data_manager.py에서 통합)
# ============================================
def update_stock_prices_in_dy_trad1(workbook, tickers, global_min_start_date, user_fetch_start_date, user_fetch_end_date):
    """yfinance에서 최신 종가를 조회하여 Google Sheet를 업데이트합니다."""
    sheet_name = "StockPrices"
    worksheet = get_or_create_worksheet(workbook, sheet_name)
    if worksheet is None:
        st.error("워크시트를 가져올 수 없습니다.")
        return

    st.info(f"'{', '.join(tickers)}'의 종가 데이터를 업데이트합니다...")

    # 1. Google Sheet에서 기존 데이터 로드
    ...
    if not df_sheet.empty:
        ...
        last_date_in_sheet = df_sheet['Date'].max() # yfinance 다운로드 시작일 최적화에 사용
    else:
        df_sheet = pd.DataFrame(columns=['Date']) # 시트가 비어있으면 'Date' 컬럼만 가진 빈 DataFrame으로 초기화
        last_date_in_sheet = None

    # Determine the actual start date for yfinance download
    user_fetch_start_dt = pd.to_datetime(user_fetch_start_date)
    user_fetch_end_dt = pd.to_datetime(user_fetch_end_date)

    # yfinance 다운로드 시작일을 최적화하여 불필요하게 오래된 데이터를 다시 다운로드하지 않도록 합니다.
    yf_download_start_dt = user_fetch_start_dt # 기본적으로 사용자가 요청한 시작일
    if last_date_in_sheet is not None:
        # 기존 데이터가 있다면, 시트의 마지막 날짜로부터 며칠 전 또는 사용자가 요청한 시작일 중 더 늦은 날짜부터 다운로드합니다.
        yf_download_start_dt = max(user_fetch_start_dt, last_date_in_sheet - timedelta(days=2))
    yf_download_start_dt = max(yf_download_start_dt, pd.to_datetime(global_min_start_date))

    yf_download_end_dt = user_fetch_end_dt + timedelta(days=1)

    df_yf_raw = yf.download(tickers, start=yf_download_start_dt.strftime('%Y-%m-%d'), end=yf_download_end_dt.strftime('%Y-%m-%d'), progress=False)
    
    if df_yf_raw.empty:
        st.warning(f"'{', '.join(tickers)}'에 대한 {yf_download_start_dt.strftime('%Y-%m-%d')}부터 {user_fetch_end_dt.strftime('%Y-%m-%d')}까지의 데이터를 yfinance에서 찾을 수 없습니다.")
        return

    # 'Close' 컬럼만 추출
    df_yf = df_yf_raw['Close']

    df_yf.reset_index(inplace=True)

    valid_tickers_in_download = []
    for col in df_yf.columns:
        if col != 'Date':
            if df_yf[col].isnull().all():
                st.warning(f"경고: 티커 '{col}'에 대한 데이터를 찾을 수 없습니다. 해당 티커는 업데이트에서 제외됩니다.")
            else:
                valid_tickers_in_download.append(col)
    
    df_yf = df_yf[['Date'] + valid_tickers_in_download]

    if df_yf.empty or len(df_yf.columns) == 1: # 'Date' 컬럼만 남았다면 유효한 티커 데이터 없음
        st.warning(f"선택하신 기간 ({user_fetch_start_date} ~ {user_fetch_end_date}) 동안 유효한 티커 데이터를 찾을 수 없습니다.")
        return

    # 종가를 소수점 둘째 자리에서 반올림
    for col in valid_tickers_in_download:
        df_yf[col] = df_yf[col].round(2)

    df_yf_for_merge = df_yf[(df_yf['Date'] >= user_fetch_start_dt) & (df_yf['Date'] <= user_fetch_end_dt)].copy()

    if df_yf_for_merge.empty:
        st.warning(f"선택하신 기간 ({user_fetch_start_date} ~ {user_fetch_end_date}) 동안 '{', '.join(valid_tickers_in_download)}'에 대한 데이터를 찾을 수 없습니다.")
        return

    # 기존 시트 데이터와 새로 가져온 yfinance 데이터를 결합합니다.
    # 이 과정에서 기존 데이터는 모두 포함되며, 겹치는 날짜에 대해서는 새로 가져온 데이터가 기존 데이터를 덮어씁니다.
    df_combined = pd.concat([df_sheet, df_yf_for_merge], ignore_index=True)

    # 'Date' 컬럼을 기준으로 중복을 제거하고, 가장 최근(새로 가져온) 데이터를 유지합니다.
    # 이후 날짜 순으로 정렬합니다.
    df_to_write = df_combined.drop_duplicates(subset=['Date'], keep='last').sort_values(by='Date').reset_index(drop=True)

    # 최종 DataFrame에 모든 예상 티커 컬럼이 포함되도록 재색인하고, 누락된 값은 NaN으로 채웁니다.
    # 이는 새로운 티커가 추가되거나 기존 티커가 제거될 때 컬럼 일관성을 유지하기 위함입니다.
    all_expected_cols = ['Date'] + sorted(list(set(df_to_write.columns) - {'Date'}))
    df_to_write = df_to_write.reindex(columns=all_expected_cols)
    
    # Google Sheet에 쓰기 위해 'Date' 컬럼을 문자열로 포맷팅
    df_to_write['Date'] = df_to_write['Date'].dt.strftime('%Y-%m-%d')
    
    # 숫자형 컬럼의 NaN 값을 빈 문자열로 채워 Google Sheet에 'nan' 대신 빈 셀로 표시
    ...
    st.success(f"'{sheet_name}' 시트가 성공적으로 업데이트되었습니다. 마지막 날짜: {df_to_write['Date'].iloc[-1]}")


# ============================================
# ✅ Google Sheet에서 가격 데이터 로드
# ============================================
@st.cache_data(ttl=120) # 2분 캐시
def load_price_data_from_sheet(sheet_name, start_date, end_date):
    """Google Sheet에서 특정 기간의 주가 데이터를 로드합니다."""
    worksheet = get_worksheet(sheet_name)
    if worksheet is None:
        st.error(f"'{sheet_name}' 워크시트를 찾을 수 없습니다.")
        st.stop()
...
# ============================================
@st.cache_data(ttl=120, show_spinner=False)  # 2분 캐시
def get_mode_and_target_prices(start_date, end_date, target_ticker, first_amt, day_cnt, style_option, 
    ...
    start_dt_pd, end_dt_pd = pd.to_datetime(start_date), pd.to_datetime(end_date)
    
    # RSI 계산을 위해 25주 이전 데이터부터 로드
    fetch_start_date = start_dt_pd - pd.Timedelta(weeks=25)

    # Google Sheet에서 데이터 로드
    all_prices = load_price_data_from_sheet("StockPrices", fetch_start_date, end_dt_pd)

    qqq = all_prices[['QQQ']].rename(columns={'QQQ': 'Close'}).dropna()
    ticker_data = all_prices[[target_ticker]].rename(columns={target_ticker: 'Close'}).dropna()

    # 거래일 캘린더
    nyse = mcal.get_calendar("NYSE")
    market_days = nyse.schedule(
        start_date=start_dt_pd.strftime("%Y-%m-%d"),
        end_date=(end_dt_pd + pd.Timedelta(days=dfns_hold_days + 60)).strftime("%Y-%m-%d")
    ).index.normalize()

    if end_dt_pd not in qqq.index: 
        # 종료일자가 데이터에 없으면 빈 행 생성 (추후 로직 유지용)
        qqq.loc[end_dt_pd] = np.nan
    ...
    for day in market_days:
        if not (start_dt_pd <= day <= end_dt_pd):
            continue
...
with col1:
    # 📝 티커 선택
    tickers = ('SOXL', 'KORU', 'TQQQ', 'RAM', 'BITU','TNA','UDOW','TECL')
...
# Google Sheet 종가 업데이트 버튼
if st.button("Google Sheet 종가 업데이트"):
    with st.spinner("Google Sheet 종가 데이터를 업데이트 중입니다..."):
        # price_data_manager.py에서 사용하던 티커 목록
        all_managed_tickers = ['QQQ', 'SOXL', 'KORU', 'TNA', 'UDOW', 'RAM','TECL', 'BITU', 'TQQQ']
        # 2011-01-01부터 전일까지의 데이터를 업데이트하도록 설정
        update_stock_prices_in_dy_trad1(workbook, all_managed_tickers, '2011-01-01', datetime(2011, 1, 1).date(), datetime.now().date() - timedelta(days=1))
