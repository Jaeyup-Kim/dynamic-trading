...
# ⭐ 백테스트 초기 투자 금액 및 시작일 설정 ⭐
INITIAL_CASH = 20000.0
TARGET_START_DATE = "2010-02-12" 
...
def get_weekly_data_from_daily(ticker, start_date):
    ...
    """
    try:
        df_daily = fdr.DataReader(ticker, start=start_date, exchange='NASDAQ')
...
def jy_tqqq_backtest():
    ...
    for i in range(1, data_points):
        ...
        if pnl > 0: # 매도
            ...
            if suggested_shares > max_buy_shares:
                warning = "현금 부족으로 조정"
        
        exec_shares = float(exec_shares) 
